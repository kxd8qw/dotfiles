...
-- Command Mode
map("c", "w!!", "silent! w !sudo tee >/dev/null '%' | e!",
...
map("n", "<leader>d", [["_d]], { desc = "Delete (don't stomp clipboard)" })
...
map("n", "&", "<cmd>&&<CR>", { desc = "Repeat last substitution" })
...
-- Visual (only) Mode
map("x", "<leader>d", [["_d]], { desc = "Delete (don't stomp clipboard)" })
map("x", "<leader>p", [["_dP]], { desc = "Paste (don't stomp clipboard)" })
map("x", "&", "<cmd>&&<CR>", { desc = "Repeat last substitution" })