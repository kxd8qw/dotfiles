...
for i in $(curl -LSs "${site}"'/'"${user}"'?tab=repositories' |
      awk -F'"' '/codeRepository/ {print $2}' | sort); do
  curl -LSs ${site}${i}/issues | awk -F'"' \
        '$2 ~ /'"$(sed 's|/|\\/|g' <<< $i)"'\/issues\/[0-9]+/ \
        {print "OPEN: '"${site}"'"$2}' || :
  curl -LSs ${site}${i}/pulls | awk -F'"' \
        '$2 ~ /'"$(sed 's|/|\\/|g' <<< $i)"'\/pull\/[0-9]+/ \
        {print "OPEN: '"${site}"'"$2}' || :
  curl -LSs "${site}${i}"'/issues?q=is%3Aissue' | awk -F'"' 'BEGIN {uri=""} \
        /@dperson/ {uri=""} $10~/\/issues\/[0-9]+/ \
        {if(uri != "") {print "UNASSIGNED: '"$site"'"uri}; uri=$10}' ||:
  curl -LSs "${site}${i}"'/pulls?q=is%3Apr' | awk -F'"' 'BEGIN {uri=""} \
