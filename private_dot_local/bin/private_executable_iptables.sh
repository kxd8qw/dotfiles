...
start () {
    ...
    if [[ "$ROUTER" == "Y" ]]; then
        ...
        $IPT -A ip-out -o $WAN -p TCP -m multiport --dports 135:139,445 \
                    -j REJECT
        $IPT -A ip-out -o $WAN -p TCP -m multiport --sports 135:139,445 \
                    -j REJECT
        $IPT -A ip-out -o $WAN -p UDP -m multiport --dports 135:139,445 \
                    -j REJECT
        $IPT -A ip-out -o $WAN -p UDP -m multiport --sports 135:139,445 \
                    -j REJECT
    fi
