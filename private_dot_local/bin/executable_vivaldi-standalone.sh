...
if [[ -z "$PKG" ]]; then
  URL="$(curl -LSfs https://vivaldi.com/download/ |
        sed -n 's/.*"\(https[^"]*'"$(uname -m).rpm\)"'".*/\1/p' | head -1)"
  PKG="/tmp/$(echo $URL | sed 's|.*/||')"
  curl -LSfs "$URL" -o "$PKG"
fi
...
if [[ -z "$VIVALDI_PKG_INFO" ]]; then
  ...
  echo "Example usage: $0 vivaldi-stable-1.4.589.29-1.$(uname -m).rpm" >&2
  exit 1
fi
...
ls vivaldi* >/dev/null 2>&1 && rm -rf vivaldi*
...
echo "$PKG" | grep '^/tmp/' && rm "$PKG"
ln -s vivaldi* vivaldi
