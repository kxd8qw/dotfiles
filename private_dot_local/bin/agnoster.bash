...
######################################################################
### Segment drawing
# A few utility functions to make it easy and re-usable to draw segmented prompt
...
# to add "256 (8-bit) Colors" follow the example for orange
fg_color() {
  case "$1" in
    black)      echo 30;;
    red)        echo 31;;
...
__command_rprompt() {
  ...
  for tz in ZRH:Europe/Zurich PIT:US/Eastern MTV:US/Pacific TOK:Asia/Tokyo; do
    [ $n -gt 40 ] || break
...
set_bash_prompt() {
  ...
  CURRENT_BG=NONE
  PRIGHT=""
  prompt_right_segment black white "$(date '+ %Y-%m-%d %H:%M ')"
  TITLE="$([[ $DEFAULT_USER != $(whoami) ]] && echo "$USER@")"
  TITLE='\[]0;'"$TITLE${HOSTNAME%%.*}:"'\w\]'
  PR="$TITLE$(ansi_single $(text_effect reset))"
  build_prompt

  # uncomment below to use right prompt
  #  PS1='\[$(tput sc; printf "%*s" $COLUMNS "$PRIGHT"; tput rc)\]'$PR
  #  PS1=$PR
  PS1='\[$(tput sc; printf "%$((COLUMNS - 20))s$PRIGHT\\r" ''; tput rc)\]'$PR
}
